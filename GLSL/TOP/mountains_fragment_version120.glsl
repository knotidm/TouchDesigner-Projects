...
vec3 noised( in vec2 x )
{
	...
	float a = texture2D(sInput1,(p+vec2(0.5,0.5))/256.0,-100.0).x;
	float b = texture2D(sInput1,(p+vec2(1.5,0.5))/256.0,-100.0).x;
	float c = texture2D(sInput1,(p+vec2(0.5,1.5))/256.0,-100.0).x;
	float d = texture2D(sInput1,(p+vec2(1.5,1.5))/256.0,-100.0).x;
...
vec3 calcNormal( in vec3 pos, float t )
{
	...
	return normalize( vec3( terrain2(pos.xz-eps.xy) - terrain2(pos.xz+eps.xy), 2.0*eps.x, terrain2(pos.xz-eps.yx) - terrain2(pos.xz+eps.yx) ) );
}
...
{
	float f = 0.0;
	f += 0.5000*texture2D( sInput1, p/256.0 ).x;
	p = m2*p*2.02;
	f += 0.2500*texture2D( sInput1, p/256.0 ).x;
	p = m2*p*2.03;
	f += 0.1250*texture2D( sInput1, p/256.0 ).x;
	p = m2*p*2.01;
	f += 0.0625*texture2D( sInput1, p/256.0 ).x;
	return f/0.9375;
}
...
void main(void)
{
	...
	else
	{
		...
		// rock
		float r = texture2D( sInput1, 7.0*pos.xz/256.0 ).x;
		col = (r*0.25+0.75)*0.9*mix( vec3(0.08,0.05,0.03), vec3(0.10,0.09,0.08), texture2D(sInput1,0.00007*vec2(pos.x,pos.y*48.0)).x );
