...
void main()
{
	...
	for (int i = 0; i < u_number_of_lights; ++i)
	{
		...
		vec3 light_position = texelFetch(u_light_positions, i).xyz;
		vec3 light_color = texelFetch(u_light_colors, i).xyz;
		vec3 light_falloff = texelFetch(u_light_falloffs, i).xyz;
