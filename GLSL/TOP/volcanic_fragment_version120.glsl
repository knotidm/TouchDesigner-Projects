...
float noise( in vec2 x )
{
	...
	return texture2D( sInput1, (uv+118.4)/256.0, -100.0 ).x;
}
...
void main(void)
{
	...
	// camera	
	float off = step( 0.001, iMouse.x )*6.0*iMouse.x/iResolution.x;
	...
	if( t>0.0 )
	{
		...
		col = texcube( sInput2, 0.5*pos, nor ).xyz;
		col = col*(0.2+0.8*texcube( sInput3, 4.0*vec3(2.0,8.0,2.0)*pos, nor ).x);
		vec3 verde = vec3(1.0,0.9,0.2);
		verde *= texture2D( sInput3, pos.xz, lodbias ).xyz;
		col = mix( col, 0.8*verde, hh );
		float vv = smoothstep( 0.0, 0.8, nor.y )*smoothstep(0.0, 0.1, pos.y-0.8 );
		verde = vec3(0.2,0.45,0.1);
		verde *= texture2D( sInput3, 30.0*pos.xz, lodbias ).xyz;
		verde += 0.2*texture2D( sInput3, 1.0*pos.xz, lodbias ).xyz;
		vv *= smoothstep( 0.0, 0.5, texture2D( sInput3, 0.1*pos.xz + 0.01*nor.x ).x );
